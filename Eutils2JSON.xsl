...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="*">
    <xsl:param name="indent" select="&quot;&quot;"/>
    <xsl:variable name="msg" select="concat(&quot;FIXME:  unmatched element &quot;, np:dq(name(.)))"/>
  ...
  <xsl:template name="term-tree">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$lastelem[self::OP]">
        ...
        <xsl:call-template name="term-tree">
          <xsl:with-param name="indent" select="concat($indent, $iu)"/>
          <xsl:with-param name="elems" select="$elems[position() &lt; $numelems - $num-top-elems]"/>
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:variable name="num-next-elems">
          <xsl:call-template name="count-top-elems">
            <xsl:with-param name="elems" select="$elems[position() &lt; last() - $num-top-elems]"/>
          </xsl:call-template>
        </xsl:variable>
