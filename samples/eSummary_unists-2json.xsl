...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="Chr | Polymorphic | Marker_Name | Gene_ID | Locus | Org | EPCR_Summary | @uid | @status">
...
<xsl:choose>
   ...
   <xsl:template match="Map_Gene_Summary_List">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="a">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="Map_Gene_Summary">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
