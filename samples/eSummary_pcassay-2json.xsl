...
<xsl:choose>
   ...
   <xsl:template match="BioSystemsList | ProteinTargetList | TopCidList | BioSystems | XRefAsurlList | XRefDburlList | TopFiveCidList | SourceNameList">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
   ...
   <xsl:template match="ProteinTarget | TopCidType">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="o">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
