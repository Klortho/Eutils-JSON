...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="citations | genes | linkedsystems | protein | taxonomy | smallmolecules | category | comment | entity | idtypes | property | names | moleculenames">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="array">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="ModifyDate | bscategory | textcitation | propertyname | linkedsystemtype | hilitetype | propertyvalue | biosystemtype | string | accession | description | linkedsystemname | taxonomyname | url | genename | feedbackurl | biosystemname | bstype | idtypesenum | externalid | source | sourceurl | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
   ...
   <xsl:template match="citation | gene | linkedsystemstruct | propertystruct | highliteurl | smallmoleculestruct | proteinstruct | genestruct | taxnodestruct | biosystem | entitystruct">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="object">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="gi | pmid | sid | SIDCount | ProteinCount | CIDCount | GeneCount | cid | taxid | bsid | geneid">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
