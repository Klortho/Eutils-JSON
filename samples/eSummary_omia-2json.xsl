...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="Author_List | Reference_List | Gene_List | PubMed_List | MP_List | Computed_Gene_List | Breed | OMIM_ID | Across_Species_Synonym_List">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="a">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="Pathology | Publisher | Molecular_Genetics | Inheritance | Genetic_Test | Species_Phenotype_Name | Publish_Place | Clinical_Feature | Sci_Name | Volume | Title | string | Journal | Common_Name | Taxonomy_ID | Pages | OMIA_ID | Curation_State | Gene_Name | Gene_ID | Prevalence | Phenotype | MP_Name | Species_Phenotype_Summary | Phenotype_Summary | MP_ID | Inherit_Text | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
...
<xsl:choose>
   ...
   <xsl:template match="MP | Gene | Reference">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
   ...
   <xsl:template match="int | Year | PubMed_UID">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
