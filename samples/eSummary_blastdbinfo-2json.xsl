...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="tag | SearchTerm | Path | SrcDb | EntrezDatabase | GenCollAssemblyName | GpipeBuild | algorithm | options | EntrezQuery | SeqStrategy | Title | WgsProject | url | TotalLength | Description | LastUpdated | SeqType | Name | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="string">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="Perc25 | MaxLength | NumSequences | Perc50 | N50 | Taxid | Perc75 | pid">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
   ...
   <xsl:template match="Project | Term | SourceDbData | ProjectData | TermData | OtherUrlData | SourceDb | OtherUrl | SequenceType | Masking">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="array">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="SequenceTypeData | MaskingData">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
