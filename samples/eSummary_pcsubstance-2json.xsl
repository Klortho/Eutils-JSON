...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="IsomericSmiles | TPSA | ModifyDate | SourceID | CurrentSourceName | MolecularWeight | MaxTC | CanonicalSmiles | MinAC | RegistryNumber | Complexity | string | DBUrl | SourceReleaseDate | XLogP | IUPACName | MaxAC | MinTC | InChIKey | SBUrl | HoldUntilDate | DepositDate | MolecularFormula | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="s">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="TotalFormalCharge | BondChiralUndefCount | SID | AtomChiralCount | InactiveAidCount | HydrogenBondAcceptorCount | BondChiralDefCount | ProbeAidCount | AtomChiralUndefCount | IsotopeAtomCount | AtomChiralDefCount | int | HeavyAtomCount | RotatableBondCount | TautomerCount | HydrogenBondDonorCount | BondChiralCount | CovalentUnitCount | ActiveAidCount | TotalAidCount">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
...
<xsl:choose>
   ...
   <xsl:template match="SynonymList | CompoundIDList | MeSHHeadingList | AssaySourceNameList | SourceCategoryList | PharmActionList | MeSHTermList | SourceNameList | CommentList">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
