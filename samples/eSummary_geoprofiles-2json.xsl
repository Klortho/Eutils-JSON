...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="summary | GBACC | UGName | ENTREZ_GENE_ID | GDS | VMIN | VMAX | groups | taxon | SPOTID | gi | outliers | GSM_L | title | geneName | gdsType | PTACC | idref | SUB_SAMP_L | valType | SUB_TYPES_L | SAGEmap | EVALUE | cloneID | ORF | nucDesc | abscalls_chain | GSE | taxid_L | ERANK | geneDesc | UGDesc | alias | GPL | abscall | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="s">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="RMEAN | AOUTL | RSTD | AFLAG">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
