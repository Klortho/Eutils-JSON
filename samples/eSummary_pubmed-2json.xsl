...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="Lang | References | PubType | ArticleIds | DocContribList | Attributes | SrcContribList | Authors | History">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="a">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="BookName | Issue | LastAuthor | Date | EPubDate | AuthType | PubDate | PublisherName | RefType | SortFirstAuthor | DocDate | AvailableFromURL | ESSN | Volume | Note | Title | RefSource | LocationLabel | Edition | string | NlmUniqueID | PubStatus | DocType | FullJournalName | BookTitle | SortTitle | ReportNumber | ClusterID | Value | RecordStatus | Pages | ELocationID | SrcDate | Source | flag | PublisherLocation | SortPubDate | Chapter | Medium | IdType | Name | ISSN | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="s">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="PubMedPubDate | Author | Reference | ArticleId">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
...
<xsl:choose>
   ...
   <xsl:template match="PmcRefCount | IdTypeN | PMID | ViewCount">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
