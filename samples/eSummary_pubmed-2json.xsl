...
<x:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <x:template match="BookName | Issue | LastAuthor | Date | EPubDate | AuthType | PubDate | PublisherName | PmcRefCount | RefType | SortFirstAuthor | DocDate | AvailableFromURL | ESSN | Volume | Note | Title | RefSource | LocationLabel | Edition | string | NlmUniqueID | IdTypeN | PubStatus | DocType | FullJournalName | BookTitle | SortTitle | ReportNumber | ClusterID | Value | RecordStatus | Pages | ELocationID | SrcDate | Source | flag | PublisherLocation | SortPubDate | Chapter | PMID | ViewCount | Medium | IdType | Name | ISSN | @uid | @status">
      <x:param name="indent" select="&#34;&#34;"/>
      <x:param name="context" select="&#34;unknown&#34;"/>
   ...
   <x:template match="PubMedPubDate | Author | Reference | ArticleId | DocumentSummarySet">
      <x:param name="indent" select="&#34;&#34;"/>
      <x:param name="context" select="&#34;unknown&#34;"/>
