...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="ScientificName | ModificationDate | Genus | CommonName | Rank | Species | Subsp | Status | Division | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="string">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="TaxId | StructNumber | AkaTaxId | ProtNumber | NucNumber | GenNumber | GeneNumber">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
