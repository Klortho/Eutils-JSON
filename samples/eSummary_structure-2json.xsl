...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="EC | DNAMoleculeCount | PdbClass | MMDBModifyDate | LigCode | PdbAcc | LigCount | string | MMDBEntryDate | RNAMoleculeCount | DNAChainCount | ModRNAResCount | ProteinMoleculeCount | ExpMethod | ProteinChainCount | PdbDescr | RNAChainCount | ModDNAResCount | Resolution | ModProteinResCount | PdbDepositDate | @uid | @status">
...
<xsl:choose>
   ...
   <xsl:template match="OrganismList | PdbAccSynList">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
