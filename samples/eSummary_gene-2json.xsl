...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="Chromosome | NomenclatureSymbol | OtherDesignations | NomenclatureName | ChrLoc | NomenclatureStatus | MapLocation | Orgname | OtherAliases | GeneticSource | ChrAccVer | Summary | Description | ChrSort | Name | @uid | @status">
...
<xsl:choose>
   ...
   <xsl:template match="GenomicInfoType">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="o">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="GeneWeight | ChrStart | ChrStop | CurrentID | TaxID | int | ExonCount | Status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
   ...
   <xsl:template match="Mim | GenomicInfo">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
