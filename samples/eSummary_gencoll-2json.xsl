...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="ReleaseLevel | Coverage | ReleaseType | Organism | NCBIReleaseDate | UCSCName | Properties | AssemblyClass | AssemblyDescription | Primary | AssemblyName | int | PartialGenomeRepresentation | AssemblyAccession | EnsemblName | WGS | Category | Meta | @uid | @status">
   ...
   <xsl:template match="GB_Projects | RS_Projects">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
