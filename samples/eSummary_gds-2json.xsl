...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="summary | GDS | subsetInfo | Accession | taxon | GEO2R | title | gdsType | URL | valType | Title | RelationType | ptechType | SSInfo | PlatformTaxa | GSE | PDAT | TargetObject | FTPLink | SeriesTitle | PlatformTitle | SamplesTaxa | suppFile | Name | GPL | entryType | @uid | @status">
...
<xsl:choose>
   ...
   <xsl:template match="Project | Sample | Relation">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="o">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="Samples | Projects | Relations | ExtRelations | PubMedIds">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
   ...
   <xsl:template match="int | n_samples">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
