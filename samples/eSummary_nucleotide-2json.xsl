...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="Extra | Organism | Caption | Strand | Biomol | Title | SubType | SourceDb | Genome | CreateDate | GeneticCode | AccessionVersion | Slen | Topology | Completeness | SubName | MolType | Tech | Meta | UpdateDate | AssemblyAcc | @est | @gss | @subtype | @popset | @na | @qual | @uid | @type | @value | @status | @genome | @trace | @source | @aa">
...
<xsl:choose>
   ...
   <xsl:template match="SegSetSize | AssemblyGi | ProjectId | TaxId | Flags | Gi | @count">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
