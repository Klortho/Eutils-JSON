...
<xsl:choose>
   ...
   <xsl:template match="Accession | RetireComment | RetireDate | Title | CreateDate | DocType | Summary | UpdateDate | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="s">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="DocStatus | TaxId | GeneId | ReplacedById">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
