...
<xsl:stylesheet xmlns:np="http://ncbi.gov/portal/XSLT/namespace"
   ...
   <xsl:template match="SiteDescriptions">
...
<xsl:choose>
   ...
   <xsl:template match="Accession | Organism | PubDate | EntrezDate | Title | string | Abstract | Database | Subtitle | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="s">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="LivePssmID | NumberSites | PssmLength | Status | StructureRepresentative">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
