...
<xsl:choose>
   ...
   <xsl:template match="Issue | EPubDate | AuthType | PubDate | SortDate | Volume | Title | FullJournalName | Value | Pages | Source | PmcLiveDate | error | IdType | Name | @uid | @status">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="s">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="Author | ArticleId">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
      <xsl:call-template name="o">
         <xsl:with-param name="context" select="$context"/>
      </xsl:call-template>
   </xsl:template>
   <xsl:template match="ArticleIds | Authors">
      <xsl:param name="context" select="&#34;unknown&#34;"/>
